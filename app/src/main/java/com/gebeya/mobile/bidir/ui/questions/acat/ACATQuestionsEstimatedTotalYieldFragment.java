...
public class ACATQuestionsEstimatedTotalYieldFragment extends BaseFragment {
    ...
    public static ACATQuestionsEstimatedTotalYieldFragment newInstance() {
        return new ACATQuestionsEstimatedTotalYieldFragment();
    }

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        root = inflater.inflate(R.layout.fragment_acat_questions_estimated_total_yield_layout, container, false);
        ...
        previousButton.setOnClickListener(v -> {
            Fragment fragment = ACATCostEstimationFragment.newInstance();
            FragmentTransaction transaction = getFragmentManager().beginTransaction();
            transaction.replace(R.id.questionsACATContainer, fragment ); // give your fragment container id in first parameter
            transaction.addToBackStack(null);  // if written, this transaction will be added to backstack
            transaction.commit();
